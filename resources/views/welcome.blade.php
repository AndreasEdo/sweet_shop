                ...
                <div class="grid grid-cols-5 grid-rows-1 gap-4 h-full">
                    @foreach ($others as $promo)
                        <div class="w-full bg-[#F8BBD052] rounded-3xl h-5/6 grid grid-rows-[1fr_2fr] relative">
                            ...
                            @if (auth()->check())
                                <a href="javascript:void(0)" data-product-id="{{ $promo->product->id }}" class="add-to-cart transition-all absolute group top-5 right-0 w-1/7 h-16 bg-amber-200 absolute rounded-l-3xl flex items-center justify-around font-bold text-black cursor-pointer hover:w-2/6">
